...
function playFloorLoop(G, cap){
  ...
  while(!G.over && t < cap){
    ...
    /* 受傷了而且附近沒東西，就坐下來休息。
       ...
       用 G.turn 有沒有動來判斷「這一步是不是真的花掉了」：
       restStep() 回 false 有兩種完全不同的意思 —— 「附近有怪，拒絕休息」
       （沒花回合）與「休息到滿了」（花了回合）。分不出來的話，
       後者會讓機器人在同一回合又走一步，等於一回合動兩次。 */
    if(p.hp * 3 < p.mhp * 2 && p.sat > 8000){
      const tn = G.turn;
      api.restStep();
      if(G.turn !== tn){ t++; continue; }
    }
